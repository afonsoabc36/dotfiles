...
local directions = {
	{ mod = {}, key = "h", frac = 1 / 60, dx = -1, dy = 0 },
	{ mod = {}, key = "l", frac = 1 / 60, dx = 1, dy = 0 },
	{ mod = {}, key = "j", frac = 1 / 60, dx = 0, dy = 1 },
	{ mod = {}, key = "k", frac = 1 / 60, dx = 0, dy = -1 },
	{ mod = { "control" }, key = "h", frac = 1 / 8, dx = -1, dy = 0 },
	{ mod = { "control" }, key = "l", frac = 1 / 8, dx = 1, dy = 0 },
	{ mod = { "control" }, key = "j", frac = 1 / 8, dx = 0, dy = 1 },
	{ mod = { "control" }, key = "k", frac = 1 / 8, dx = 0, dy = -1 },
}
...
bindScrollKey("down", { 0, -scrollLargeStep }, { 0, -scrollStep }, function()
	moveMouseByFraction(0, dragMoveLargeFrac)
end, function()
	moveMouseByFraction(0, dragMoveFrac)
end)
bindScrollKey("up", { 0, scrollLargeStep }, { 0, scrollStep }, function()
	moveMouseByFraction(0, -dragMoveLargeFrac)
end, function()
	moveMouseByFraction(0, -dragMoveFrac)
end)
...
for _, dir in ipairs(arrowDirections) do
	local xFrac, yFrac = dir.dx * dir.frac, dir.dy * dir.frac
	bindHoldWithDelay({}, dir.key, function()
		moveMouseByFraction(xFrac, yFrac)
	end, directionInitialDelay, directionRepeatInterval)
end
...
modal:bind({}, "m", function()
	performClicks(2, false)
end)
modal:bind({}, "n", function()
	performClicks(1, false)
end)
